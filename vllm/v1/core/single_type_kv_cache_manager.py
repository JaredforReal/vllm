...
class SingleTypeKVCacheManager(ABC):
    ...
    def __init__(
        ...
        # inert at decode), so the virtual block size scales with dcp only.
        # This must match block_table's slot kernel (TOTAL_CP == dcp).
        if dcp_world_size > 1:
...
class FullAttentionManager(SingleTypeKVCacheManager):
    ...
    def find_longest_cache_hit(
        ...
        # Cache sharded by DCP only (PCP replicated); see __init__.
        if dcp_world_size > 1:
