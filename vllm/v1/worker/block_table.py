...
class BlockTable:
    ...
    def compute_slot_mapping(
        ...
        # KV cache is sharded by DCP only. PCP uses a replicated cache (PCP is
        # a prefill-compute optimization, inert at decode), so PCP ranks do not
        # partition KV slots -- every PCP rank holds the full cache and decode
        # runs the normal path. Backward-compatible: pcp_world_size==1 gives the
        # same values as the prior ``pcp*dcp`` formula. (Sharding the cache by
        # total_cp + a decode Mode-2 LSE merge is the ascend-style path; it is
        # disabled here pending H100 debugging -- see _forward_decode_pcp.)
...
class MultiGroupBlockTable:
    ...
    def __init__(
        ...
        if max_num_blocks is None:
            # Note(hc): each dcp rank only stores
            # (max_model_len//dcp_world_size) tokens in the kvcache, so the
            # block_size used to calc max_num_blocks_per_req must be multiplied
            # by dcp_world_size. PCP uses a replicated cache (does not reduce
            # per-rank block allocation).
